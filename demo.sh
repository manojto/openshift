...
#    --showlog=true
  ...
  TKN pipeline start build-and-deploy \
    -w name=shared-workspace,volumeClaimTemplateFile=01_pipeline/03_persistent_volume_claim.yaml \
    -p pipelines-hello-manoj \
    -p git-url=https://github.com/manojto/hello-manoj.git \
    -p IMAGE="image-registry.openshift-image-registry.svc:5000/openshift/hello-manoj:latest" \
    --use-param-defaults \
    --showlog=true
